...
function generateDigestA(plaintext, conf) {
    ...
    // steps 1-8
    const hashA = crypto_1.createHash(HashType[conf.id]);
    hashA.update(plaintext);
    hashA.update(conf.saltString);
    const hashB = crypto_1.createHash(HashType[conf.id]);
...
function generateHash(plaintext, conf) {
    ...
    // steps 13-15
    const plaintextByteLength = buffer_1.Buffer.byteLength(plaintext);
    const hashDP = crypto_1.createHash(hashType);
    ...
    // step 17-19
    const hashDS = crypto_1.createHash(hashType);
    ...
    const digestC = rounds.reduce((acc, curr, idx) => {
        const hashC = crypto_1.createHash(hashType);
        // steps b-c
        if (idx % 2 === 0) {
            hashC.update(acc);
        }
...
function verify(plaintext, hash) {
    ...
    return crypto_1.timingSafeEqual(buffer_1.Buffer.from(computedHash, "utf8"), buffer_1.Buffer.from(hash, "utf8"));
}
